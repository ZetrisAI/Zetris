...
namespace Zetris {
    static class GameHelper {
        public static ProcessMemory Game { get; private set; } = new ProcessMemory("puyopuyotetris", false);
        ...
        public static CachedMethod<int, byte> BagIndex = new CachedMethod<int, byte>((index) =>
            Game.TraverseByte(
                new IntPtr(0x140461B20),
                InSwap.Call()
                    ? new int[] {0x378 + index * 0x8, 0x30, 0x300, 0x3D8}
                    : new int[] {0x378 + index * 0x8, 0xA8, 0x3D8}
            )?? 0
        );
